...
func (h ChatHeadler) SendMessage(c *gin.Context) {
	// 解析请求参数
...
func (h ChatHeadler) GetMessages(c *gin.Context) {
	// 获取路径参数
	uuid := c.Param("uuid")
	if uuid == "" {
	...
	// 获取消息列表
	messages, total, err := service.Chat.GetMessages(uuid, page, pageSize)
	if err != nil {
...
func (h ChatHeadler) GetConversation(c *gin.Context) {
	// 获取路径参数
	uuid := c.Param("uuid")
	if uuid == "" {
		response.BadRequest(c, "缺少对话UUID", nil)
	...
	// 获取对话信息
	conversation, err := service.Chat.GetConversationByUUID(uuid)
	if err != nil {
