    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        if (fullMessage.type === 'new_conversation') {
          ...
          // 添加详细的调试日志
          console.log('当前会话ID(ref):', currentConversationIdRef.current, '类型:', typeof currentConversationIdRef.current);
          console.log('当前会话ID(state):', currentConversationId, '类型:', typeof currentConversationId);
          console.log('消息会话ID:', data.conversation_id, '类型:', typeof data.conversation_id);
          console.log('ID比较结果:', data.conversation_id === currentConversationIdRef.current);
          
          // 判断消息是否属于当前打开的会话
          if (messageData && data.conversation_id === currentConversationIdRef.current) {
      ...
      <Router>
        <div className="h-full flex flex-col p-[24px]">
          ...
          <main className="flex-1 container mx-auto px-4 py-8 h-[calc(100vh-60px)] overflow-auto">
            <Routes>
              <Route path="/" element={<Chat onCurrentConversationChange={(conversationId: number) => {
